...
import com.ulusoy.allaboutmaps.main.common.MathUtil.EARTH_RADIUS
import com.ulusoy.allaboutmaps.main.common.MathUtil.arcHav
import com.ulusoy.allaboutmaps.main.common.MathUtil.havDistance
...
object SphericalUtil {
    ...
    fun computeHeading(from: LatLng, to: LatLng): Double {
        ...
        val toLat = Math.toRadians(to.latDoubleValue)
        val toLng = Math.toRadians(to.lngDoubleValue)
        val dLng = toLng - fromLng
        val heading = atan2(
            sin(dLng) * cos(toLat),
            cos(fromLat) * sin(toLat) - sin(fromLat) * cos(
                toLat
            ) * cos(dLng)
        )
    ...
    fun computeOffsetOrigin(to: LatLng, distance: Double, heading: Double): LatLng? {
        ...
        val n4 = sin(Math.toRadians(to.latDoubleValue))
        ...
        val fromLngRadians = Math.toRadians(to.lngDoubleValue) -
                atan2(
                    n3,
                    n1 * cos(fromLatRadians) - n2 * sin(fromLatRadians)
                )
        return LatLng(
            Math.toDegrees(fromLatRadians),
    ...
    fun interpolate(from: LatLng, to: LatLng, fraction: Double): LatLng {
        ...
        if (sinAngle < 1E-6) {
            return LatLng(
                from.latDoubleValue + fraction * (to.latDoubleValue - from.latDoubleValue),
                from.lngDoubleValue + fraction * (to.lngDoubleValue - from.lngDoubleValue)
            )
        }
        ...
        val x =
            a * cosFromLat * cos(fromLng) + b * cosToLat * cos(toLng)
        val y =
            a * cosFromLat * sin(fromLng) + b * cosToLat * sin(toLng)
    ...
    fun computeAngleBetween(from: LatLng, to: LatLng): Double {
        return distanceRadians(
            Math.toRadians(from.latDoubleValue), Math.toRadians(from.lngDoubleValue),
            Math.toRadians(to.latDoubleValue), Math.toRadians(to.lngDoubleValue)
        )
    }
    ...
    fun computeLength(path: List<LatLng>): Double {
        ...
        var length = 0.0
        val prev: LatLng = path[0]
        var prevLat = Math.toRadians(prev.latDoubleValue)
        var prevLng = Math.toRadians(prev.lngDoubleValue)
        for (point in path) {
            val lat = Math.toRadians(point.latDoubleValue)
            val lng = Math.toRadians(point.lngDoubleValue)
    ...
    fun computeSignedArea(
        ...
        var total = 0.0
        val prev: LatLng = path[size - 1]
        var prevTanLat =
            tan((Math.PI / 2 - Math.toRadians(prev.latDoubleValue)) / 2)
        var prevLng = Math.toRadians(prev.lngDoubleValue)
        // For each edge, accumulate the signed area of the triangle formed by the North Pole
        // and that edge ("polar triangle").
        for (point in path) {
            val tanLat =
                tan((Math.PI / 2 - Math.toRadians(point.latDoubleValue)) / 2)
            val lng = Math.toRadians(point.lngDoubleValue)
