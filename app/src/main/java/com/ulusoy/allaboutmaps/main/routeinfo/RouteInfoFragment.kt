...
import com.ulusoy.allaboutmaps.databinding.FragmentRouteInfoBinding
import com.ulusoy.allaboutmaps.main.MapProvider
import com.ulusoy.allaboutmaps.main.routeinfo.mapbox.RouteInfoMapboxFragmentDirections
import dagger.android.support.DaggerFragment

class RouteInfoFragment : DaggerFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val navController = requireActivity().findNavController(R.id.route_info_nav_host_fragment)
        binding.bottomNav.setupWithNavController(navController)
        (arguments?.getSerializable("mapProvider") as? MapProvider)?.let {
            when (it) {
                MapProvider.HUAWEI -> navController.navigate(
                    RouteInfoMapboxFragmentDirections.actionMapboxFragmentToHuaweiFragment()
                )
                MapProvider.GOOGLE -> navController.navigate(
                    RouteInfoMapboxFragmentDirections.actionMapboxFragmentToGoogleFragment()
                )
                else -> {
